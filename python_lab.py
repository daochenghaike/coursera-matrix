...
## Task 15
exclude_zero_list = [y for y in [(x,y,z) for x in S for y in S for z in S if x+y+z == 0] if y != (0,0,0)]

## Task 16
first_of_tuples_list = [y for y in [(x,y,z) for x in S for y in S for z in S if x+y+z == 0] if y != (0,0,0)][0]
...
base = 10
digits = set(range(10))
representation_dict = {x:[x // base**2, x % base**2 // 10, x % base] for x in range(1000)}
